...
def pivot_months_pandas(data):
    ...
    grouped_data_monthly = grouped_data.aggregate({'precipitation': 'sum'}).reset_index()
    monthly = grouped_data_monthly.pivot_table(index='name', columns='month', values='precipitation', aggfunc='sum')

    grouped_data_count = data.groupby(['name', 'month']).size().reset_index(name='counts')
    counts = grouped_data_count.pivot_table(index='name', columns='month', values='counts', aggfunc='sum')
